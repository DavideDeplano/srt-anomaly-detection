...
class AEHyperModel(kt.HyperModel):
    ...
    ----------
    h_total : int
        Input height after reshaping: 6 * H (panels concatenated vertically).
    w : int
        Input width (frequency bins, typically 80).
    random_state : int
        Random seed for TensorFlow.
    """

    def __init__(self, h_total: int, w: int, random_state: int = 42) -> None:
    ...
    def build(self, hp: kt.HyperParameters) -> keras.Model:
        ...
        h2, w2 = self.h_total // 2, self.w // 2
        h4, w4 = self.h_total // 4, self.w // 4

        # Encoder
        inputs = keras.Input(shape=(self.h_total, self.w, 1), name="encoder_input")
        ...
        outputs = layers.Conv2D(1, (3, 3), padding="same", activation="sigmoid", name="decoder_output")(x)
